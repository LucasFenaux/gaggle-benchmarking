...
def run_tiny_network():
    ...
    dir = 'Results/'
    filename = 'tiny_cartpole_leap_pop_size_{}.p'.format(args.population_size)
...
def run_small_network():
    ...
    times = timing_probe.buffer
    dir = 'Results/'
    filename = 'small_cartpole_leap_pop_size_{}_{}.p'.format(args.population_size, device)
    if not os.path.exists(dir):
        os.makedirs(dir)
    with open(os.path.join(dir, filename), 'wb') as f:
        pickle.dump(times, f)
...
def run_medium_network():
    ...
    times = timing_probe.buffer
    dir = 'Results/'
    filename = 'medium_cartpole_leap_pop_size_{}_{}.p'.format(args.population_size, device)
    if not os.path.exists(dir):
        os.makedirs(dir)
    with open(os.path.join(dir, filename), 'wb') as f:
        pickle.dump(times, f)
